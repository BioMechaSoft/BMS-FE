  ...
  return (
    <Modal isOpen={isOpen} onRequestClose={onClose} ariaHideApp={false} style={{ content: { maxWidth: 700, margin: 'auto', borderRadius: 8, background: '#fff' } }}>
      <div style={{ padding: 24 }}>
        <h2 style={{ marginBottom: 16 }}>Invoice <span style={{ color: '#0859af' }}>{invoice?.invoiceNumber || invoice?._id || ''}</span></h2>
        {invoice ? (
          <div>
            <div style={{ marginBottom: 8 }}>
              <b>Patient:</b> {invoice.patient ? `${invoice.patient.firstName || ''} ${invoice.patient.lastName || ''}` : '-'}<br/>
